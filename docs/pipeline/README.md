...
This repository includes comprehensive test coverage reporting using [pytest-cov](https://pytest-cov.readthedocs.io/). Coverage reports help ensure code quality and identify untested code paths.

#### Running Tests with Coverage

**For pipeline trigger code:**

```bash
make test-trigger-coverage
```

**For specific component:**

```bash
make test-components-coverage GROUP=vertex-components
```

**For all components:**

```bash
make test-all-components-coverage
```
...
- **Exclusions**: Test files, cache directories, and non-source files are excluded from coverage

#### Coverage Reports

After running tests with coverage, you can view detailed reports:

- **HTML Report**: Open `pipelines/htmlcov/index.html` or `components/*/htmlcov/index.html` in your browser
- **Terminal Output**: Shows coverage percentage and lists missing lines
- **XML Report**: Machine-readable format for CI/CD integration

#### Coverage Commands Reference

| Command                                      | Description                           |
| -------------------------------------------- | ------------------------------------- |
| `make test-trigger-coverage`                 | Run trigger tests with coverage       |
| `make test-components-coverage GROUP=<name>` | Run component tests with coverage     |
| `make test-all-components-coverage`          | Run all component tests with coverage |
| `make coverage-report`                       | Generate combined coverage report     |

#### Direct pytest Commands

You can also run coverage directly with pytest:

```bash
# In pipelines directory
cd pipelines && pipenv run pytest --cov=src --cov-report=html

# In component directory
cd components/vertex-components && pipenv run pytest --cov=src --cov-report=html
```
