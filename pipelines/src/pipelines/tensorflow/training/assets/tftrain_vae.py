...
DEFAULT_HPARAMS = dict(
    ...
    latent_dim=3,
    seq_length=50,
    input_width=50,       # replaces seq_length for dataset building
    label_width=1,        # 1 = single point; set to 50 for multi-step-50
    shift=0,              # 0 = labels start immediately after inputs
    forecast_steps=None,  # keep this for manifest consistency; we will set it from label_width if None
    n_features=8,
    kl_init_weight=1e-4,
    patience=10,
    metrics=[
...
def _write_latent_tfrecords(
    xy_ds: tf.data.Dataset,          # dataset yielding (x, y)
    encoder: tf.keras.Model,
    out_dir: str,
    num_shards: int = 16,
    compression: str = "GZIP",
    ...
    """
    Encode input windows with the VAE encoder and write sharded TFRecords
    that contain z, z_mean, z_log_var and y per example.
    - x: [B, L, F]  -> encoder(x)
    - y: [B, H]     -> written as labels
    """
    tf.io.gfile.makedirs(out_dir)
    opts = tf.io.TFRecordOptions(compression)
    writers = [
    ...
    for batch_x, batch_y in xy_ds:  # x: [B, L, F], y: [B, H]
...
def main():
    ...
    # Normalize windowing params
    if hparams.get("forecast_steps") is None:
        hparams["forecast_steps"] = int(hparams["label_width"])
    # Ensure seq_length equals the input window for the VAE
    hparams["seq_length"] = int(hparams.get("input_width", hparams["seq_length"]))

    # Build sequence datasets [batch, seq_length, n_features]
    # Build (x, y) datasets with true future labels
    ...
    train_ds = train_xy.map(lambda x, y: x)
    val_ds   = val_xy.map(lambda x, y: x)
    test_ds  = test_xy.map(lambda x, y: x)
    ...
    try:
        ...
        def _write_split(split_name, xy):
            ...
            _write_manifest(
                ...
                forecast_steps=int(hparams["label_width"]),   # H in manifest
                compression="GZIP",
                latent_key="z",
                mean_key="z_mean",
                logvar_key="z_log_var",
        ...
        _write_split("train", train_xy)
        _write_split("valid", val_xy)
        _write_split("test",  test_xy)
