...
def tensorflow_pipeline(
    ...
    with dsl.ParallelFor(dc_thresholds, parallelism=0) as thr:
        ...
        #     train_data=windowing_no_dc_task.outputs["train_windowed"],
        #     valid_data=windowing_no_dc_task.outputs["valid_windowed"],
        #     test_data=windowing_no_dc_task.outputs["test_windowed"],
        #     project_id=project_id,
        #     project_location=project_location,
        ...
        #     staging_bucket=staging_bucket,
        #     requirements=["google-cloud-logging", "google-cloud-aiplatform[cloud_profiler]","tensorboard_plugin_profile"],
        # ).set_caching_options(enable_caching=True).set_display_name("Train Forecast No DC")
        ...
        custom_forecast_train_job(
            ...
            staging_bucket=staging_bucket,
            requirements=["google-cloud-logging", "google-cloud-aiplatform[cloud_profiler]","tensorboard_plugin_profile"],
        ).set_caching_options(enable_caching=True).set_display_name("Train Forecast DC")

        custom_train_job(
            train_script_uri=latent_forecast_train_script_uri,
            train_data=train_vae.outputs["z_train"],
            valid_data=train_vae.outputs["z_valid"],
            test_data=train_vae.outputs["z_test"],
            project_id=project_id,
            project_location=project_location,
            # model_display_name=cfg.exp_name,
            train_container_uri="us-docker.pkg.dev/vertex-ai/training/tf-cpu.2-17.py310:latest",
            serving_container_uri="us-docker.pkg.dev/vertex-ai/prediction/tf2-cpu.2-15:latest",
            hparams={
                **hparams,
            ...
            threshold=thr,
            staging_bucket=staging_bucket,
            scaler_artifact=x_splits.outputs["scaler_artifact"],
            # BigQuery parameters
            dataset_id=dataset_id,
            # table_id=f"model_predictions_{thr}",
            load_to_bq=False,
            requirements=["google-cloud-logging", "pyarrow", "google-cloud-aiplatform[cloud_profiler]","tensorboard_plugin_profile"],
        ).after(train_vae).set_display_name("Train Latent Forecast")

    # joint_latent_concat = concat_latent_datasets(
    #     train_latents=dsl.Collected(train_vae.outputs["z_train"]),
    #     valid_latents=dsl.Collected(train_vae.outputs["z_valid"]),
    #     test_latents=dsl.Collected(train_vae.outputs["z_test"]),
    #     scalers=dsl.Collected(x_splits.outputs["scaler_artifact"]),
    # ).set_display_name("Concat Cross-Threshold Latents")

    joint_latent_concat = concat_latent_vectors(
        train_latents=dsl.Collected(train_vae.outputs["z_train"]),
        valid_latents=dsl.Collected(train_vae.outputs["z_valid"]),
        test_latents=dsl.Collected(train_vae.outputs["z_test"]),
    ).set_caching_options(enable_caching=True).set_display_name("Concat X-Threshold Latents")

    scalers = dsl.Collected(x_splits.outputs["scaler_artifact"])
    reference_scaler = pick_reference_scaler(scalers=scalers).set_display_name("Collect Scalers")

    joint_latent_forecast = custom_train_job(
        train_script_uri=latent_forecast_train_script_uri,
        ...
        hparams={
            **hparams,
            ...
            "latent_key": "z",
            "z_mean_key": "z_mean",
            "z_log_var_key": "z_log_var",
            "label_key": "y",
            "latent_dim": 12,  
        },
        threshold=0.0, # sentinel value to indicate that the threshold is not used
        staging_bucket=staging_bucket,
        # scaler_artifact=x_splits.outputs["scaler_artifact"],
        # scaler_artifact=joint_latent_concat.outputs["reference_scaler"],
        scaler_artifact=reference_scaler.outputs["reference"],
        # BigQuery parameters
        dataset_id=dataset_id,
        table_id=f"cross_threshold_model_predictions",
        load_to_bq=False,
        requirements=["google-cloud-logging", "pyarrow", "google-cloud-aiplatform[cloud_profiler]","tensorboard_plugin_profile"],
    ).after(joint_latent_concat).set_display_name("Train Joint Latent Forecast")
    ...
    ).set_caching_options(enable_caching=True).set_display_name("Concat X-Threshold Features")
    ...
    no_dc_no_vae_features = concat_threshold_features(
        train_datasets=dsl.Collected(windowing_no_dc_task.outputs["train_windowed"]),
        valid_datasets=dsl.Collected(windowing_no_dc_task.outputs["valid_windowed"]),
        test_datasets=dsl.Collected(windowing_no_dc_task.outputs["test_windowed"]),
        feature_names=[
        "PRICE_std",
        "vol_quote_std",
        "cvd_quote_std",
