"""Live bridge: mainnet price data → MultiScaleDCStrategy → trades.

Connects to Hyperliquid mainnet WebSocket for real midprice ticks,
runs the Multi-Scale DC strategy (sensor-informed momentum filtering),
...
Usage:
    uv run --package hyperliquid-trading-bot python \
        trading-agent/src/strategies/dc_overshoot/multi_scale_bridge.py \
        --symbol SOL \
        --sensor-thresholds 0.002,0.004,0.008 --trade-threshold 0.015 \
        --momentum-alpha 0.3 --min-momentum-score 0.3 \
        --position-size 300 --sl-pct 0.015 --tp-pct 0.005 \
        --trail-pct 0.3 --min-profit-to-trail-pct 0.002 \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Multi-Scale DC: mainnet price data → sensor-informed strategy → trade execution"
    )
    parser.add_argument(
        "--symbol", type=str, default=DEFAULT_SYMBOL,
        help="Asset to trade (default: SOL). Must match Hyperliquid symbol.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--trade-threshold", type=float, default=0.015,
        help="Trade threshold (default: 0.015 = 1.5%%)",
    )
    parser.add_argument(
        "--momentum-alpha", type=float, default=0.3,
        help="Momentum EMA alpha (default: 0.3, ~5-event window)",
    )
    parser.add_argument(
        "--min-momentum-score", type=float, default=0.3,
        help="Min momentum score to confirm trade (default: 0.3)",
    )
    # Risk params (same as live_bridge.py)
    parser.add_argument(
        "--position-size", type=float, default=100.0,
        help="Position size in USD (default: 100)",
    )
    parser.add_argument(
        "--sl-pct", type=float, default=0.015,
        help="Initial stop loss %% as decimal (default: 0.015 = 1.5%%)",
    )
    parser.add_argument(
        "--tp-pct", type=float, default=0.005,
        help="Initial take profit %% as decimal (default: 0.005 = 0.5%%)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--min-profit-to-trail-pct", type=float, default=0.002,
        help="Min raw profit before trailing ratchet activates (default: 0.002 = 0.2%%)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Enable structured telemetry (writes NDJSON to GCS or local fallback)",
    )
    parser.add_argument(
        "--telemetry-dir", type=str, default=None,
        help="Custom local telemetry output directory",
    )
    return parser.parse_args()


def parse_sensor_thresholds(s: str) -> list[tuple[float, float]]:
    """Parse comma-separated sensor thresholds into list of (down, up) tuples."""
    return [(float(v), float(v)) for v in s.split(",")]


# ---------------------------------------------------------------------------
# Adapter + backstop helpers (same as live_bridge.py)
# ---------------------------------------------------------------------------
...
async def place_backstop_sl(adapter, symbol: str, side: str, entry_price: float,
    ...
    try:
        ...
        result = adapter.exchange.order(
            name=symbol, is_buy=is_buy, sz=rounded_size,
...
async def execute_signal(adapter, signal, current_price: float) -> bool:
    ...
    try:
        if signal.signal_type == SignalType.CLOSE:
            ...
            result = adapter.exchange.order(
                name=signal.asset, is_buy=is_buy, sz=rounded_size,
            ...
            order = Order(
                id=f"ms_bridge_{int(time.time()*1000)}",
                asset=signal.asset,
                side=side,
...
async def main():
    ...
    sensor_thresholds = parse_sensor_thresholds(args.sensor_thresholds)
    ...
    if network == "mainnet" and not args.observe_only and not args.yes:
        ...
        print("!" * 70)
        print(f"  Strategy: Multi-Scale DC")
        print(f"  Wallet:   {account_address or 'API wallet (no delegation)'}")
        print(f"  Sensors:  {sensor_thresholds}")
        print(f"  Trade:    {args.trade_threshold}")
        print(f"  Momentum: alpha={args.momentum_alpha} min_score={args.min_momentum_score}")
        print(f"  SL/TP:    {args.sl_pct*100:.1f}% / {args.tp_pct*100:.1f}%  Size: ${args.position_size}")
        print()
    ...
    # Build multi-scale strategy config
    config = {
        "symbol": symbol,
        "sensor_thresholds": list(sensor_thresholds),
        "trade_threshold": [args.trade_threshold, args.trade_threshold],
    ...
    strategy = MultiScaleDCStrategy(config)
    strategy.start()
    ...
    # Register DC event callback for telemetry (includes is_sensor flag from strategy)
    def _on_dc_event(event: dict) -> None:
        telem.emit(EventType.DC_EVENT, event)
        # Emit MOMENTUM_UPDATE for sensor events
        if event.get("is_sensor"):
            telem.emit(EventType.MOMENTUM_UPDATE, {
                "score": strategy._scorer.score,
                "regime": strategy._scorer.get_regime(),
                "threshold_key": f"{event.get('threshold_down')}:{event.get('threshold_up')}",
    ...
    logger.info("=" * 70)
    logger.info("Multi-Scale DC Live Bridge")
    logger.info("=" * 70)
    ...
    logger.info("Sensors    : %s", sensor_thresholds)
    logger.info("Trade      : %.4f (%.2f%%)", args.trade_threshold, args.trade_threshold * 100)
    logger.info("Momentum   : alpha=%.2f min_score=%.2f", args.momentum_alpha, args.min_momentum_score)
                ...
                try:
                    async for message in ws:
                        ...
                        for signal in signals:
                            ...
                            # Emit signal telemetry (with momentum data)
                            telem.emit(EventType.SIGNAL, {
                                ...
                                "momentum_score": signal.metadata.get("momentum_score"),
                        ...
                        # Status log every 100 ticks (with momentum info)
                        if tick_count % 100 == 0:
                            elapsed = ts - start_time
                            rm_status = strategy._trailing_rm.get_status()
                            scorer_status = strategy._scorer.get_status()
                            score = scorer_status.get("score", 0.0)
                            regime = scorer_status.get("regime", "?")
                            pos_info = ""
                            ...
                            logger.info(
                                "Tick #%d | price=%.2f | score=%.3f (%s) | "
                                "sensors=%d filt=%d signals=%d trades=%d | "
                                "elapsed=%.0fs %s%s%s",
                                tick_count, price, score, regime,
                                strategy._sensor_event_count, strategy._filtered_count,
    ...
    telem.emit(EventType.SESSION_END, {
        ...
        "signal_count": signal_count,
        "trade_count": trade_count,
        "sensor_event_count": status["sensor_event_count"],
        "trade_event_count": status["trade_event_count"],
        "filtered_count": status["filtered_count"],
        "reconnect_count": reconnect_count,
        "final_momentum_score": status["scorer"]["score"],
        "final_regime": status["scorer"]["regime"],
    })
    telem.close()
    ...
    logger.info("Sensor events  : %d", status["sensor_event_count"])
    logger.info("Trade events   : %d", status["trade_event_count"])
    logger.info("Filtered       : %d", status["filtered_count"])
    logger.info("Signals        : %d", signal_count)
    logger.info("Trades         : %d", trade_count)
    logger.info("Reconnects     : %d", reconnect_count)
    logger.info("Momentum score : %.3f (%s)", status["scorer"]["score"], status["scorer"]["regime"])
