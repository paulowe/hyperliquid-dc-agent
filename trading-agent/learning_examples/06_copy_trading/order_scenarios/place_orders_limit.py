...
ORDER_SIZE = 2.5  # Size to meet minimum $10 USDC requirement
PRICE_OFFSET_PCT = 0  # 50% below market for buy order (won't fill)
...
SCENARIOS = {
    ...
    # Time-limited orders: expires_after field means reject this action if it arrives after this timestamp (in milliseconds)
    # 9: {
...
async def place_limit_orders():
    ...
    try:
        ...
        # Get sz decimals for the base token
        for token in spot_meta.get("tokens", []):
                if token.get("name") == SYMBOL.split("/")[0]:
        ...
        for scenario_id, scenario in SCENARIOS.items():
            ...
            try:
                ...
                order_size, order_price = format_order(ORDER_SIZE, order_price, sz_decimals=sz_decimals, is_spot=True, is_buy=is_buy)
