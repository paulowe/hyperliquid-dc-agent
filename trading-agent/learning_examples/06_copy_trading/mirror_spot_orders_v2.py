...
Key refactor: "fast path" parsing + "slow path" execution
- Fast path: receive WS messages -> parse JSON -> filter -> enqueue "actions" (NO network I/O)
- Slow path: execute actions sequentially (order/cancel), but run sync HTTP in a thread using asyncio.to_thread()
  so the event loop stays responsive and the WS receiver doesn't starve.

Why this matters for latency + correctness:
- If you do sync HTTP inside the event loop, you stop draining the WebSocket.
- You may miss order updates, stop seeing "Message added", or get disconnected under bursty traffic.
- Offloading sync HTTP keeps the WS receiver running continuously, while still preserving strict execution order.
...
async def execute_place(exchange: Exchange, info: Info, action: Action) -> Optional[int]:
    ...
    asset_info = await get_spot_price_and_szdec(info, action.coin)
    if not asset_info:
    ...
    print(f"🔄 Placing follower order: {'BUY' if is_buy else 'SELL'} {order_size} {action.coin} @ ${price}")
...
async def monitor_and_mirror_spot_orders():
    ...
    # Preload spot meta cache once (helps latency later)
    try:
        await load_spot_cache(info)
    except Exception as e:
